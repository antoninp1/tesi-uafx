...
#define SERVER_PUBLIC_URL "opc.tcp://192.168.17.75:4841"
...
int main(void) {
    ...
    /* ─── Registrazione all'LDS (Sintassi open62541 v1.4.x) ─── */

    // 1. Creiamo una configurazione Client vuota di default
    UA_ClientConfig cc;
    memset(&cc, 0, sizeof(UA_ClientConfig));
    UA_ClientConfig_setDefault(&cc);

    // 2. Definiamo l'indirizzo del nostro server LDS (sostituisci con l'IP della VPN)
    UA_String discoveryUrl = UA_STRING(LDS_URL);

    // 3. Avviamo la registrazione periodica
    // Il server si registrerà ora e ripeterà l'operazione in automatico
    ...
    if(retval_lds != UA_STATUSCODE_GOOD) {
        printf("⚠ Errore durante la registrazione all'LDS: %s\n", UA_StatusCode_name(retval_lds));
    } else {
        printf("✓ Registrazione LDS configurata con successo!\n");
    }
    printf("════════════════════════════════════════════════════\n");
    printf("  SERVER RUNNING\n");
    printf("════════════════════════════════════════════════════\n");
    printf("Endpoint:        opc.tcp://192.168.100.3:4840\n");
