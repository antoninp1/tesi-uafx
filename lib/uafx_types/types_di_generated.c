/**********************************
 * Autogenerated -- do not modify *
 **********************************/

#include "types_di_generated.h"

/* DeviceHealthEnumeration */
#define DeviceHealthEnumeration_members NULL

/* TransferResultErrorDataType */
static UA_DataTypeMember TransferResultErrorDataType_members[2] = {
{
    UA_TYPENAME("Status") /* .memberName */
    &UA_TYPES[UA_TYPES_INT32], /* .memberType */
    0, /* .padding */
    false, /* .isArray */
    false  /* .isOptional */
},
{
    UA_TYPENAME("Diagnostics") /* .memberName */
    &UA_TYPES[UA_TYPES_DIAGNOSTICINFO], /* .memberType */
    offsetof(UA_TransferResultErrorDataType, diagnostics) - offsetof(UA_TransferResultErrorDataType, status) - sizeof(UA_Int32), /* .padding */
    false, /* .isArray */
    false  /* .isOptional */
},};

/* ParameterResultDataType */
static UA_DataTypeMember ParameterResultDataType_members[3] = {
{
    UA_TYPENAME("NodePath") /* .memberName */
    &UA_TYPES[UA_TYPES_QUALIFIEDNAME], /* .memberType */
    0, /* .padding */
    true, /* .isArray */
    false  /* .isOptional */
},
{
    UA_TYPENAME("StatusCode") /* .memberName */
    &UA_TYPES[UA_TYPES_STATUSCODE], /* .memberType */
    offsetof(UA_ParameterResultDataType, statusCode) - offsetof(UA_ParameterResultDataType, nodePath) - sizeof(void *), /* .padding */
    false, /* .isArray */
    false  /* .isOptional */
},
{
    UA_TYPENAME("Diagnostics") /* .memberName */
    &UA_TYPES[UA_TYPES_DIAGNOSTICINFO], /* .memberType */
    offsetof(UA_ParameterResultDataType, diagnostics) - offsetof(UA_ParameterResultDataType, statusCode) - sizeof(UA_StatusCode), /* .padding */
    false, /* .isArray */
    false  /* .isOptional */
},};

/* SoftwareClass */
#define SoftwareClass_members NULL

/* LocationIndicationType */
#define LocationIndicationType_members NULL

/* SoftwareVersionFileType */
#define SoftwareVersionFileType_members NULL

/* UpdateBehavior */
#define UpdateBehavior_members NULL

/* TransferResultDataDataType */
static UA_DataTypeMember TransferResultDataDataType_members[3] = {
{
    UA_TYPENAME("SequenceNumber") /* .memberName */
    &UA_TYPES[UA_TYPES_INT32], /* .memberType */
    0, /* .padding */
    false, /* .isArray */
    false  /* .isOptional */
},
{
    UA_TYPENAME("EndOfResults") /* .memberName */
    &UA_TYPES[UA_TYPES_BOOLEAN], /* .memberType */
    offsetof(UA_TransferResultDataDataType, endOfResults) - offsetof(UA_TransferResultDataDataType, sequenceNumber) - sizeof(UA_Int32), /* .padding */
    false, /* .isArray */
    false  /* .isOptional */
},
{
    UA_TYPENAME("ParameterDefs") /* .memberName */
    &UA_TYPES_DI[UA_TYPES_DI_PARAMETERRESULTDATATYPE], /* .memberType */
    offsetof(UA_TransferResultDataDataType, parameterDefsSize) - offsetof(UA_TransferResultDataDataType, endOfResults) - sizeof(UA_Boolean), /* .padding */
    true, /* .isArray */
    false  /* .isOptional */
},};
UA_DataType UA_TYPES_DI[UA_TYPES_DI_COUNT] = {
/* DeviceHealthEnumeration */
{
    UA_TYPENAME("DeviceHealthEnumeration") /* .typeName */
    {0, UA_NODEIDTYPE_NUMERIC, {6244LU}}, /* .typeId */
    {0, UA_NODEIDTYPE_NUMERIC, {0}}, /* .binaryEncodingId */
    sizeof(UA_DeviceHealthEnumeration), /* .memSize */
    UA_DATATYPEKIND_ENUM, /* .typeKind */
    true, /* .pointerFree */
    UA_BINARY_OVERLAYABLE_INTEGER, /* .overlayable */
    0, /* .membersSize */
    DeviceHealthEnumeration_members  /* .members */
},
/* TransferResultErrorDataType */
{
    UA_TYPENAME("TransferResultErrorDataType") /* .typeName */
    {0, UA_NODEIDTYPE_NUMERIC, {15888LU}}, /* .typeId */
    {0, UA_NODEIDTYPE_NUMERIC, {15891LU}}, /* .binaryEncodingId */
    sizeof(UA_TransferResultErrorDataType), /* .memSize */
    UA_DATATYPEKIND_STRUCTURE, /* .typeKind */
    false, /* .pointerFree */
    false, /* .overlayable */
    2, /* .membersSize */
    TransferResultErrorDataType_members  /* .members */
},
/* ParameterResultDataType */
{
    UA_TYPENAME("ParameterResultDataType") /* .typeName */
    {0, UA_NODEIDTYPE_NUMERIC, {6525LU}}, /* .typeId */
    {0, UA_NODEIDTYPE_NUMERIC, {6554LU}}, /* .binaryEncodingId */
    sizeof(UA_ParameterResultDataType), /* .memSize */
    UA_DATATYPEKIND_STRUCTURE, /* .typeKind */
    false, /* .pointerFree */
    false, /* .overlayable */
    3, /* .membersSize */
    ParameterResultDataType_members  /* .members */
},
/* SoftwareClass */
{
    UA_TYPENAME("SoftwareClass") /* .typeName */
    {0, UA_NODEIDTYPE_NUMERIC, {408LU}}, /* .typeId */
    {0, UA_NODEIDTYPE_NUMERIC, {0}}, /* .binaryEncodingId */
    sizeof(UA_SoftwareClass), /* .memSize */
    UA_DATATYPEKIND_ENUM, /* .typeKind */
    true, /* .pointerFree */
    UA_BINARY_OVERLAYABLE_INTEGER, /* .overlayable */
    0, /* .membersSize */
    SoftwareClass_members  /* .members */
},
/* LocationIndicationType */
{
    UA_TYPENAME("LocationIndicationType") /* .typeName */
    {0, UA_NODEIDTYPE_NUMERIC, {410LU}}, /* .typeId */
    {0, UA_NODEIDTYPE_NUMERIC, {0}}, /* .binaryEncodingId */
    sizeof(UA_LocationIndicationType), /* .memSize */
    UA_DATATYPEKIND_UINT16, /* .typeKind */
    true, /* .pointerFree */
    UA_BINARY_OVERLAYABLE_INTEGER, /* .overlayable */
    0, /* .membersSize */
    LocationIndicationType_members  /* .members */
},
/* SoftwareVersionFileType */
{
    UA_TYPENAME("SoftwareVersionFileType") /* .typeName */
    {0, UA_NODEIDTYPE_NUMERIC, {331LU}}, /* .typeId */
    {0, UA_NODEIDTYPE_NUMERIC, {0}}, /* .binaryEncodingId */
    sizeof(UA_SoftwareVersionFileType), /* .memSize */
    UA_DATATYPEKIND_ENUM, /* .typeKind */
    true, /* .pointerFree */
    UA_BINARY_OVERLAYABLE_INTEGER, /* .overlayable */
    0, /* .membersSize */
    SoftwareVersionFileType_members  /* .members */
},
/* UpdateBehavior */
{
    UA_TYPENAME("UpdateBehavior") /* .typeName */
    {0, UA_NODEIDTYPE_NUMERIC, {333LU}}, /* .typeId */
    {0, UA_NODEIDTYPE_NUMERIC, {0}}, /* .binaryEncodingId */
    sizeof(UA_UpdateBehavior), /* .memSize */
    UA_DATATYPEKIND_UINT32, /* .typeKind */
    true, /* .pointerFree */
    UA_BINARY_OVERLAYABLE_INTEGER, /* .overlayable */
    0, /* .membersSize */
    UpdateBehavior_members  /* .members */
},
/* TransferResultDataDataType */
{
    UA_TYPENAME("TransferResultDataDataType") /* .typeName */
    {0, UA_NODEIDTYPE_NUMERIC, {15889LU}}, /* .typeId */
    {0, UA_NODEIDTYPE_NUMERIC, {15892LU}}, /* .binaryEncodingId */
    sizeof(UA_TransferResultDataDataType), /* .memSize */
    UA_DATATYPEKIND_STRUCTURE, /* .typeKind */
    false, /* .pointerFree */
    false, /* .overlayable */
    3, /* .membersSize */
    TransferResultDataDataType_members  /* .members */
},
};

